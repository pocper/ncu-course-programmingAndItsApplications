...
namespace Ex03_DeadLock
{
    public class Account
    {
        ...
        public static void Transfer(Account from, Account to, int amount)
        {
            ...
            Account firstLockedAccount = fromId > toId ? from : to;
            Account secondLockedAccount = fromId > toId ? to : from;

            var thrdId = Thread.CurrentThread.ManagedThreadId;
            Console.WriteLine(string.Format("Thrd#{3}: 預備從帳戶{0}轉帳到帳戶{1}，轉帳金額{2}", from.ID, to.ID, amount, thrdId));
            Console.WriteLine(string.Format("Thrd#{1}: 預備lock 資源: {0}", firstLockedAccount.ID, thrdId));
            lock (firstLockedAccount)
            {
                Console.WriteLine(string.Format("Thrd#{1}: 已經lock 資源: {0}", firstLockedAccount.ID, thrdId));

                Console.WriteLine(string.Format("Thrd#{1}: 預備lock 資源: {0}", secondLockedAccount.ID, thrdId));
                lock (secondLockedAccount)
                {
                    Console.WriteLine(string.Format("Thrd#{1}: 已經lock 資源: {0}", secondLockedAccount.ID, thrdId));
                    from.Withdraw(amount);
                    to.Deposit(amount);

                }
                Console.WriteLine(string.Format("Thrd#{1}: unlock 資源: {0}", secondLockedAccount.ID, thrdId));
            }
            Console.WriteLine(string.Format("Thrd#{1}: unlock 資源: {0}", firstLockedAccount.ID, thrdId));
